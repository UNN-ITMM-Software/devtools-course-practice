...
class HuffmanApp {
 public:
	 ...
	 std::string encode(int argc, char** argv) {
		 ...
		 _message << encoded << std::endl;

		 for (const auto& i : codes) {
			 _message << i.first << " " << i.second << std::endl;
		 }
	 ...
	 std::string decode(int argc, char** argv) {
		 ...
		 _message << huffman.decode(argv[1], _getCodes(argc, argv)) << std::endl;
 ...
 private:
	 ...
	 void _help(const std::string& appName, const std::string* message)
	 {
		 _message << "	This is an application that allows you to \
