...
std::vector<std::pair<double, double>> GrahamScan::sortPoints
    ...
    std::sort(points.begin() + 1, points.end(), [&points](std::pair<double,
     double>& a, std::pair<double, double>& b) {
...
std::vector<std::pair<double, double>> GrahamScan::convexHull
                        (std::vector<std::pair<double, double>>& points) {
    int n = points.size();
 ...
 [](const std::pair<double, double>& a, const std::pair<double, double>& b) {
    ...
    if (collinear) {
        return std::vector<std::pair<double, double>>{sortedPoints[0],
                                                     sortedPoints[n-1]};
    }
    ...
    for (int i = 2; i < n; i++) {
        while (hull.size() > 1 && orientation(hull[hull.size() - 2],
         hull[hull.size() - 1], sortedPoints[i]) != 2) {
            hull.pop_back();
        }
