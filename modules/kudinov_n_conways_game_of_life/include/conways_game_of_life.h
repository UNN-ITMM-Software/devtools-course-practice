...
#ifndef MODULES_KUDINOV_N_CONWAYS_GAME_OF_LIFE_INCLUDE_CONWAYS_GAME_OF_LIFE_
#define MODULES_KUDINOV_N_CONWAYS_GAME_OF_LIFE_INCLUDE_CONWAYS_GAME_OF_LIFE_
...
class ConwaysGameOfLife {
  private:
    ...
    std::size_t _calculateBoardIndexFromCellPosition(std::size_t y, std::size_t x) const;
  public:
    ConwaysGameOfLife(std::size_t board_height, std::size_t board_width);
    ConwaysGameOfLife(const std::vector<std::vector<bool>>& cells);
...
#endif  // MODULES_KUDINOV_N_CONWAYS_GAME_OF_LIFE_INCLUDE_CONWAYS_GAME_OF_LIFE_
