...
std::string Base64Encoder::Encode() const {
  ...
  for (size_t sixthIndex = 0; sixthIndex < sixthsCount; ++sixthIndex) {
    ...
    encodedMessage.push_back(
        internal::Base64Alphabet::GetInstance().GetSymbolByIndex(number));
  }
...
std::vector<char> Base64Encoder ::ConvertStringToBytesByAddingEmptyBytes()
    const {
...
std::string Base64Decoder::Decode() const {
  ...
  if (alsoBytesNeedToDecode != 0) {
    auto message = "Encoded message must as least have " +
                   std::to_string(alsoBytesNeedToDecode) + " bytes to decode";
  ...
  for (size_t decodedByteIndex = 0; decodedByteIndex < bytesFromDecodedMessage;
       decodedByteIndex++) {
...
Base64Decoder::ConvertEncodedMessageBytesFromBase64Alphabet() const {
  ...
  auto countOfSpecialByteSymbol =
      std::count(_encodedMessage.begin(), _encodedMessage.end(), '=');
  ...
  auto expectOnlySpecialSymbolsStringEndOfEncoded =
      _encodedMessage.substr(_encodedMessage.size() - countOfSpecialByteSymbol,
  ...
  return std::move(
      std::make_pair(std::move(converted), countOfSpecialByteSymbol));
}
