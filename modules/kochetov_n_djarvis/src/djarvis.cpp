...
int ConvexHull::orientation(Point p, Point q, Point r) {
    ...
    return (val > 0) ? 1 : -1;  // положительный или отрицательный поворот
}
...
std::vector<Point> ConvexHull::findConvexHull() {
    ...
    // Добавляем крайнюю левую точку в выпуклую оболочку и ищем следующую точку
    // до тех пор, пока не вернемся в исходную точку
    do {
        hull.push_back(points[p]);
        q = (p + 1) % n;

        // Находим следующую точку такую, что она формирует положительный поворот
