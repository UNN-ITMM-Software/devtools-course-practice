...
BullsCowsApplication::BullsCowsApplication()
    : appName("Bulls and Cows"), errorCode(EC_OK) {}
...
int BullsCowsApplication::charToInt(const char* arg) { return std::atoi(arg); }
    ...
    if (result.first == difficulty) {
      ...
      if (game.getAttemptsLeft() > 0) {
        ...
        message << "Sorry, you've run out of attempts. "
                << "The secret number was: " << game.generateSecretNumber()
                << "\n";
      }
    }
  }
