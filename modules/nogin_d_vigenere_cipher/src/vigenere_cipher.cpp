...
std::string vigenereCipher(const std::string& data, const std::string& key) {
  ...
  for (int i = 0; i < lenData; ++i) {
    ...
    if ((data[i] >= var::upperCaseBegin) && (data[i] <= var::lowerCaseEnd) &&
      ...
      new_symbol = data[i] + transformKey[(i - space) % lenKey] - var::lowerCaseBegin;
      if ((data[i] <= var::upperCaseEnd) && (data[i] >= var::upperCaseBegin)) {
        while (new_symbol > var::upperCaseEnd)
          new_symbol -= var::alphabetSize;
      } else if ((data[i] <= var::lowerCaseEnd) && (data[i] >= var::lowerCaseBegin)) {
...
std::string vigenereDecipher(const std::string& cipher, const std::string& key) {
  ...
  for (int i = 0; i < lenCipher; ++i) {
    unsigned char new_symbol;
    if ((cipher[i] >= var::upperCaseBegin) && (cipher[i] <= var::lowerCaseEnd) &&
        ((cipher[i] <= var::upperCaseEnd) || (cipher[i] >= var::lowerCaseBegin))) {
      new_symbol = cipher[i] - transformKey[(i - space) % lenKey] + var::lowerCaseBegin;
      if ((cipher[i] <= var::upperCaseEnd) && (cipher[i] >= var::upperCaseBegin)) {
        while (new_symbol < var::upperCaseBegin)
          new_symbol += var::alphabetSize;
      } else if ((cipher[i] <= var::lowerCaseEnd) && (cipher[i] >= var::lowerCaseBegin)) {
