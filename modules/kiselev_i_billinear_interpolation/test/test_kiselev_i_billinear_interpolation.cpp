...
TEST(Kiselev_Igor_Billinear_interpolation_test, test_color_constructors_and_getters) {
...
TEST(Kiselev_Igor_Billinear_interpolation_test, test_billinear_interpolation_constructors) {
...
TEST(Kiselev_Igor_Billinear_interpolation_test, test_twoCore_is_no_fall) {
    std::vector<std::vector<Color>> v(3, std::vector<Color>(3, Color(0, 0, 0)));
    BillinearInterpolation bi1 = BillinearInterpolation(v);
    ASSERT_NO_THROW(bi1.twoCore());
}

TEST(Kiselev_Igor_Billinear_interpolation_test, test_twoCore_is_correct) {
    std::vector<std::vector<Color>> v(2, std::vector<Color>(2, Color(40, 40, 40)));
    v[1][1] = Color(60, 60, 60);
    v[1][0] = Color(50, 50, 50);
    v[0][1] = Color(50, 50, 50);
    BillinearInterpolation bi1 = BillinearInterpolation(v);
    std::vector<std::vector<Color>> ans(1, std::vector<Color>(1, Color(50, 50, 50)));
...
TEST(Kiselev_Igor_Billinear_interpolation_test, test_threeCore_is_correct) {
    ...
    ASSERT_EQ(res[0][0].getG(), ans);
    ASSERT_EQ(res[0][0].getB(), ans);
}
