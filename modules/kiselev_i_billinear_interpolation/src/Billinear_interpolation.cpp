...
BillinearInterpolation::BillinearInterpolation(const BillinearInterpolation& bi) {
...
std::vector<std::vector<Color>> BillinearInterpolation::twoCore() {
	...
	n--;
	m--;
	std::vector<std::vector<Color>> ans = std::vector<std::vector<Color>>(n, std::vector<Color>(m, Color(0, 0, 0)));
    for (size_t i = 0; i < n; i++) {
        for (size_t j = 0; j < m; j++) {
...
std::vector<std::vector<Color>> BillinearInterpolation::threeCore() {
    ...
    n--;
    m--;
    std::vector<std::vector<Color>> ans = std::vector<std::vector<Color>>(n - 1, std::vector<Color>(m - 1, Color(0, 0, 0)));
...
Color BillinearInterpolation::Calculation(size_t x1, size_t y1, size_t x2, size_t y2) {
