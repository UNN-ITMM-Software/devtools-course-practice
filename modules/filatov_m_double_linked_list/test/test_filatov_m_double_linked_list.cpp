...
TEST(List_iterator_tests, correctly_iterator) {
  ...
  for (const auto& element : l) {
    if (element.check_data() == 1)
      f1 = !f1;
    else if (element.check_data() == 1 && f1)
      ADD_FAILURE();
    else if (element.check_data() == 2)
      f2 = !f2;
    else if (element.check_data() == 2 && f2)
      ADD_FAILURE();
    else if (element.check_data() == 3)
      f3 = !f3;
    else if (element.check_data() == 3 && f3)
      ADD_FAILURE();
  }
  EXPECT_EQ(f1, true);
  EXPECT_EQ(f2, true);
