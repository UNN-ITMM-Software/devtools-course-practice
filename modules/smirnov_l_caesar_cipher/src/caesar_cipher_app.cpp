...
bool CaesarCipherApplication::Validate(int argc, char *argv[]) {
    ...
    if (argc == 1) {
        ...
        Help(argv[0]);
        return false;
    } else if (std::strcmp(argv[2], "encryption") != 0 && std::strcmp(argv[2], "decryption") != 0){
...
void CaesarCipherApplication::Help(const char *application, const char *errMsg) {
    ...
    message << '\t' << " <operation> is the current operation, which can take the value 'encryption' and 'decryption'\n";
    message << '\t' << " <string> is the current line that the program is working with\n";
    msg = message.str();
}

std::string CaesarCipherApplication::CaesarCipherCryptography(int argc, char *argv[]) {
