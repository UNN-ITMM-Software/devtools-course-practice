    ...
    for (int j = 1; j <= static_cast<int>(str2.size()); j++) {
      ...
      matrix[i][j] =
          std::min(matrix[i - 1][j] + 1,
                   std::min(matrix[i][j - 1] + 1, matrix[i - 1][j - 1] + cost));
    }
  }
...
std::string LevenshteinDistance::getStr1() { return str1; }

std::string LevenshteinDistance::getStr2() { return str2; }

int LevenshteinDistance::getResult() { return result; }
