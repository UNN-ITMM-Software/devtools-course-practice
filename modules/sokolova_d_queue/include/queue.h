...
Queue<ValueType>::Queue(size_t size)
    : buffer(new ValueType[size]), head(0), tail(0), count(0), capacity(size) {}
...
template <class ValueType>
void Queue<ValueType>::enqueue(const ValueType& element) {
  if (isFull()) {
...
Queue<ValueType>::Queue(Queue&& other) noexcept
    : buffer(nullptr), head(0), tail(0), count(0), capacity(0) {
  buffer = other.buffer;
  head = other.head;
  tail = other.tail;
  count = other.count;
