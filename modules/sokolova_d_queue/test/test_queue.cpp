...
TEST(SokolovaDaria_QueueTest, CanEnqueueAndDequeueElement) {
  Queue<int> q(1);
  q.enqueue(10);
...
TEST(SokolovaDaria_QueueTest, ThrowsExceptionFrontOrBackOfEmptyQueue) {
  Queue<std::string> q;
...
TEST(SokolovaDaria_QueueTest, IsFullAfterEnqueueToCapacity) {
  Queue<int> q(2);
  q.enqueue(1);
  q.enqueue(2);
...
TEST(SokolovaDaria_QueueTest, SizeIncreasesAfterEnqueue) {
  Queue<std::string> q;
  q.enqueue("one");
  q.enqueue("two");
  size_t actualSize = 2;
...
TEST(SokolovaDaria_QueueTest, CanAssignQueues) {
  Queue<int> q1;
  q1.enqueue(100);
  q1.enqueue(200);
  size_t actualSize = 2;
...
TEST(SokolovaDaria_QueueTest, ThrowsExceptionDequeueFromEmptyQueue) {
  Queue<double> q;
  EXPECT_THROW(q.dequeue(), std::out_of_range);
}
