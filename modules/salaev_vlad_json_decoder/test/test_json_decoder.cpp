...
TEST(Salaev_Vlad_JsonDecoderTest, CanArrayDecode) {
  ...
  EXPECT_EQ(result.objectValues["ArrayTest"].arrayValues[1].type,
            JsonType::Number);
  EXPECT_EQ(result.objectValues["ArrayTest"].arrayValues[1].numberValue, 1);
...
TEST(Salaev_Vlad_JsonDecoderTest, CanNestedArrayDecode) {
  ...
  EXPECT_EQ(result.objectValues["ArrayTest"].arrayValues[1].type,
            JsonType::Array);
  EXPECT_EQ(result.objectValues["ArrayTest"]
                .arrayValues[1]
                .arrayValues[0]
                .stringValue,
            "array");
...
TEST(Salaev_Vlad_JsonDecoderTest, CanNestedObjectDecode) {
  ...
  EXPECT_EQ(result.objectValues["ObjectTest"].objectValues["object"].type,
            JsonType::Object);
  EXPECT_EQ(result.objectValues["ObjectTest"]
                .objectValues["object"]
                .objectValues["int"]
                .type,
            JsonType::Number);
...
TEST(Salaev_Vlad_JsonDecoderTest, SupportEscapedCharacters) {
  std::string json =
      R"({"Coordinate \"X\"": 24.0124010872935, "Coordinate \"Y\"":49.7740722529036})";
  size_t index = 0;
...
TEST(Salaev_Vlad_JsonDecoderTest, ParseAllDataTypes) {
  std::string json =
      R"({"str": "string", "num": 1, "bool": true, "null": null, "array": [], "object": {}})";
  size_t index = 0;
  JsonValue result = parseJson(json, index);
