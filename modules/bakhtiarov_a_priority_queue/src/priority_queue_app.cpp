...
void PriorityQueueApplication::Help(const char *application,
                                    const char *errMsg) {
  ...
  message << '\t'
          << "<operation> is the queue operation ('push', 'pop', 'front', "
             "'back')\n";
  message << '\t' << "[<value> <priority>] are required for 'push' operation\n";
  msg = message.str();
}

std::string PriorityQueueApplication::ProcessQueueOperations(int argc,
                                                             char *argv[]) {
...
std::string PriorityQueueApplication::ProcessMultipleOperations(int argc,
                                                                char *argv[]) {
