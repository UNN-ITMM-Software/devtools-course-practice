...
bool PolNot::number(char symv) {
  if ((symv >= '0') && (symv <= '9')) return true;
  return false;
}

int PolNot::prior(Lex tmp) {
  if (tmp.getS() == " ") return -1;
  if (tmp.getS() == "(" || tmp.getS() == ")") return 0;
...
TQueue<P_Lex>* PolNot::sEOL(std::string _s) {
  ...
  for (size_t i = 0; i < _s.length(); i++) {
    ...
    if (st == q0) {
      str = symv;
      if (number(symv)) st = q1;

      if (!number(symv)) q->push(new Lex(str, operation, -1));
    } else {
      if (st == q1) {
        if (number(symv)) {
          str += symv;
    ...
    if (st == q0) {
      ...
      if (tmp->getS() == "(") {
        ...
        if (stack->isEmpty()) {
          ...
          Lex* ltmp = stack->pop();
          while (!stack->isEmpty() && prior(*ltmp) <= prior(*tmp) &&
                 prior(*ltmp) != 0) {
