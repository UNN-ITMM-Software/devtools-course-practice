...
TEST(que, create_empty) { ASSERT_NO_THROW(TQueue<int> a); }
...
TEST(Lex, create_lex2) { ASSERT_NO_THROW(new Lex("/", operation, -1)); }
TEST(Lex, create_lex3) { ASSERT_ANY_THROW(new Lex("1", vall, -1)); }
TEST(Lex, create_lex4) { ASSERT_NO_THROW(new Lex("", operation, -1)); }
TEST(Lex, create_lex5) { EXPECT_ANY_THROW(new Lex("/", operation, 2)); }
TEST(Lex, getVal) {
  Lex z("/", operation, -1);
