...
TEST_P(mukhin_i_test_cases_vector, can_dot_vectors) {
  auto helper = GetParam();
  double res = (helper.v1[0] * helper.v2[0] + helper.v1[1] * helper.v2[1] +
                helper.v1[2] * helper.v2[2]);
...
TEST_P(mukhin_i_test_cases_vector, can_norm_vectors) {
  auto helper = GetParam();
  double res1 =
      std::sqrt(helper.v1[0] * helper.v1[0] + helper.v1[1] * helper.v1[1] +
                helper.v1[2] * helper.v1[2]);
  double res2 =
      std::sqrt(helper.v2[0] * helper.v2[0] + helper.v2[1] * helper.v2[1] +
...
INSTANTIATE_TEST_CASE_P(
    mukhin_i_parametric, mukhin_i_test_cases_vector,
    testing::Values(TestCaseHelper({1.0, 2.0, 3.0}, {1.0, 2.0, 3.0}),
                    TestCaseHelper({-1.0, -2.0, -3.0}, {1.0, 2.0, 3.0}),
                    TestCaseHelper({0.25, -0.75, 3.123}, {1.23, 2.89, 3.23})));
