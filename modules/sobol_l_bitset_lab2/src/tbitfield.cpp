...
TBitField::TBitField(int len) {
  if (len < 0) throw std::invalid_argument("Invalid BitField size");
  BitLen = len;
...
void TBitField::SetBit(const int n) {  // установить бит
  if ((n >= BitLen) || (n <= -1)) throw std::out_of_range("Wrong position");
...
int TBitField::GetBit(const int n) const {  // получить значение бита
  if ((n >= BitLen) || (n <= -1)) throw std::out_of_range("Wrong position");
  int ans = 0;
  if ((pMem[GetMemIndex(n)] & GetMemMask(n)) != 0) ans = 1;
  return ans;
}
